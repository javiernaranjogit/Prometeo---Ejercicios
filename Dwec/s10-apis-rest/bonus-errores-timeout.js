...
async function pedirPost(id, ms) {
    ...
    try {
        ...
        console.log(`OK id=${id}:`, post.title ?? "(sin título)");
        return post;
    } catch (error) {
        console.error(`FALLO id=${id} (timeout=${ms}ms):`, error.message);
        return null;
    }
}

async function demo() {
    await pedirPost(1, 5000);                          // ok
    await pedirPost(999, 5000);                        // HTTP 404
    await pedirPost(1, 1);                             // timeout forzado
    await pedirPost("https://dominio-inexistente.xyz", 3000); // error de red
}
